...
export function importMdtDungeon(key: DungeonKey, dungeonPath: string) {
  const data = fs.readFileSync(path.join(mdtSourceDir, `${dungeonPath}.lua`))
  ...
  const luaEnemies = dungeonEnemiesItem.init[0].fields as TableKey[]

  const enemies: Mob[] = []
  for (const luaEnemy of luaEnemies) {
    const enemyIndexVal = parseExpression(luaEnemy.key as Expression)
    if (!Number.isInteger(enemyIndexVal) || enemyIndexVal < 1) {
      throw new Error(`Invalid enemy index ${enemyIndexVal} in ${dungeonPath}.lua`)
    }

    const fields = (luaEnemy.value as TableConstructorExpression).fields as TableKey[]
    const enemy: Mob = {
  ...
  fs.writeFileSync(
    path.join(outputDir, `${key}_mdt.json`),
