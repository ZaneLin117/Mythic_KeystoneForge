...
export function CollabPanel({ collapsed }: Props) {
  ...
  useEffect(() => {
    ...
    if (collabRoom && room !== collabRoom) {
      ...
      dispatch(addToast({ message: t('collab.joined', { room: collabRoom }) }))
    }
  }, [dispatch, room, t])
  ...
  const onShare = useCallback(async () => {
    await shareUrl(room)
    dispatch(addToast({ message: t('collab.urlCopied') }))
  }, [dispatch, room, shareUrl, t])

  const onRejoin = useCallback(() => {
    dispatch(joinCollab(room))
    dispatch(addToast({ message: t('collab.rejoined', { room }) }))
