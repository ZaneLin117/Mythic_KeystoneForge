...
const rectSize = 2
const cachedWclRoutes = import.meta.glob<{ default: WclResult }>(
  '../../../server/cache/wclRoute/*.json',
)
...
export function WclCoordinateTest() {
  ...
  useEffect(() => {
    ...
    const loadRoute = cachedWclRoutes[`../../../server/cache/wclRoute/${code}-${fightId}.json`]
    if (!loadRoute) {
      setWclResult(null)
      return
    }
    loadRoute()
      .then(({ default: result }) => setWclResult(result))
      .catch(() => setWclResult(null))
  }, [route.wclUrlInfo])
